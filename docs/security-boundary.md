...
允许公开明细的子项目默认发现最近 Git 仓库内的上层依赖声明。这明确增加了子目录外的有限元数据读取：最多32层、四种受支持文件名各取最近一份、每份128 KiB。显式 deny/summary-only 祖先项目、隐藏路径和链接不读取。只有安全校验后的包名及仓库相对来源进入待审简报，不带版本、URL、脚本、原文或上层源码/文档；共享声明不产生子项目直接依赖边。可用 `discover_shared_dependencies: false` 关闭，发布前仍需审核批准。
...
自定义 `.md` 必须同时列入 `allow_files` 与 `attach_files`；默认元数据发现范围不变。附件路径拒绝隐藏/敏感名称、父目录穿越与链接，沿用逐行脱敏和体积限制。自定义文档不自动提取为指令、行动状态或项目关系，`SKILL.md` 也不例外。显式选择并不证明内容适合公开，仍需审核原文及候选包。

`python_import_roots` 只影响已扫描模块的静态解析，不扩大读取范围、不执行 Python、不改变隐藏/生成目录排除规则。多义依赖保留为未确定。解析失败只记录相对文件身份和受限错误类别，不记录可能包含源码、凭据或绝对路径的异常消息。
